...
if __name__ == "__main__":
    ...
    if resultados_base is not None and resultados_demanda_80 is not None and resultados_demanda_120 is not None:
        ...
        print(f"Valor óptimo con demanda al 80%: ${resultados_demanda_80['valor_objetivo']:,.2f}")
        print(f"Valor óptimo con demanda al 120%: ${resultados_demanda_120['valor_objetivo']:,.2f}")
        
        # Generar informe comparativo
        print("\nGenerando informe comparativo...")
        informe_80 = generar_informe_comparativo(resultados_base, resultados_demanda_80)
        informe_120 = generar_informe_comparativo(resultados_base, resultados_demanda_120)
        
        print(f"\nSe han generado los gráficos comparativos en: {OUTPUT_DIR}")
        print(f"Se han generado los informes en: {RESPUESTAS_DIR}/pregunta6_demanda_80.md y {RESPUESTAS_DIR}/pregunta6_demanda_120.md")
        
        # Mostrar un resumen comparativo básico
        for label, resultados in [("80%", resultados_demanda_80), ("120%", resultados_demanda_120)]:
            total_Y_base = sum(resultados_base['Y'].values())
            total_Y_demanda = sum(resultados['Y'].values())
            cambio_procesamiento = ((total_Y_demanda - total_Y_base) / total_Y_base) * 100 if total_Y_base > 0 else float('inf')
            
            total_NS_base = sum(resultados_base['NS'].values())
            total_NS_demanda = sum(resultados['NS'].values())
            cambio_demanda_insatisfecha = ((total_NS_demanda - total_NS_base) / total_NS_base) * 100 if total_NS_base > 0 else float('inf')
            
            print(f"\nResumen comparativo para demanda al {label}:")
            print(f"- Procesamiento de ropa en mal estado: {total_Y_base:.2f} kg (base) vs {total_Y_demanda:.2f} kg (demanda {label}) - Cambio: {cambio_procesamiento:+.2f}%")
            print(f"- Demanda insatisfecha: {total_NS_base:.2f} prendas (base) vs {total_NS_demanda:.2f} prendas (demanda {label}) - Cambio: {cambio_demanda_insatisfecha:+.2f}%")
            print(f"- Costo total: ${resultados_base['valor_objetivo']:,.2f} (base) vs ${resultados['valor_objetivo']:,.2f} (demanda {label}) - Cambio: {((resultados['valor_objetivo'] - resultados_base['valor_objetivo']) / resultados_base['valor_objetivo']) * 100:+.2f}%")
    else:
