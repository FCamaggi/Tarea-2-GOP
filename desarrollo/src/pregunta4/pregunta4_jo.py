...
def resolver_modelo_base():
    ...
    cf = param['cf']  # Costo adquisición de género ($) por kg
    ...
    G_adq = pulp.LpVariable("G_adq", lowBound=0)  # Kg de género comprados al inicio

    
    # Definir función objetivo
    model += pulp.lpSum([W[t] * ct + cc * h * w0 + g * Y[t] + n * Z[t] + 
                         a * (IB[t] + IM[t] + IG[t]) + cp * NS[t] for t in range(1, T+1)+cf * G_adq])
